 ...
 * Returns: destroy() that kills the timeline and reverts SplitText.
 */
export function initRotatingWords() {
  ...
  function step(prevIndex, currIndex) {
    ...
    tl.to(prev.chars, { y: 100, ease: 'expo.in', duration: exitDur, stagger: charStagger, overwrite: 'auto' }, '>');
    tl.fromTo(
      curr.chars,
      { y: -100 },
      { y: 0, ease: 'expo.out', duration: enterDur, stagger: charStagger, overwrite: 'auto', immediateRender: false },
      `<+=${chase}`
    );
    tl.to({}, { duration: hold });
  }
  ...
  return function destroy() {
    tl.kill();
    splits.forEach((s) => { if (s && typeof s.revert === 'function') s.revert(); });
  };
}
